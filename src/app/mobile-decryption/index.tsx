...
const MobileDecryption:React.FC<MobileDecryptionProps> = ({domain,back})=>{
    ...
    switch(page){
            case PAGES.CONTENT_ON_COMPUTER:
                    return(<ContentOnComputer initialContent={content} cancel={back} contentOnMobile={contentOnMobile} startDecrypt={startDecrypt}/>);
            case PAGES.CONTENT_ON_MOBILE:
                    return (<ContentOnMobile initialContent={content} cancel={back} contentOnComputer={contentOnComputer} startDecrypt={startDecrypt}/>);
            case PAGES.START_DECRYPT:
                    return (<DecryptContent content={content} showOnComputer={showOnComputer} contentOnComputer={contentOnComputer} />);
